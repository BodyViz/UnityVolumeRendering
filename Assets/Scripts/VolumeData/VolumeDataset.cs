			...
			try
			{
				...
				if (isHalfFloat)
				{
					...
					await Task.Run(() =>
					{
						...
						{
							progressHandler.ReportProgress(z, dimZ, "Copying slice data.");
							for (int y = 0; y < dimY; y++)
							{
								int flippedY = (dimY - 1) - y;
								for (int x = 0; x < dimX; x++)
								{
									int sourceIndex = x + flippedY * dimX + z * dimX * dimY;
									int destIndex = x + y * dimX + z * dimX * dimY;

									pixelBytes[destIndex] = Mathf.FloatToHalf(
										(float)(data[sourceIndex] - minValue) / maxRange);
								}
							}
						}
					});
